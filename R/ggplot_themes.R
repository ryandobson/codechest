...
jermeys_theme <- function(base_size = 24, base_family = "") {
  ggplot2::theme_grey(base_size = base_size, base_family = base_family) + #%+replace%
    ggplot2::theme(
      ...
      legend.text = ggplot2::element_text(size = base_size * 0.8, color = "black", family = base_family),
      legend.title = ggplot2::element_text(size = base_size * 0.8, face = "bold", color = "black", family = base_family),
      ...
      strip.background = ggplot2::element_rect(fill = "#ffffff", color = "grey10"),
      strip.text.x = ggplot2::element_text(size = base_size * 0.8, color = "black", family = base_family),
      strip.text.y = ggplot2::element_text(size = base_size * 0.8, color = "black", angle = -90, family = base_family),
      plot.background = ggplot2::element_rect(color = "#ffffff", fill = "#ffffff"),
      plot.title = ggplot2::element_text(size = base_size * 1.2, color = "black", family = base_family),
...
theme_black <- function(base_size = 24, base_family = "") {
  ggplot2::theme_grey(base_size = base_size, base_family = base_family) + #%+replace%
    ggplot2::theme(
      ...
      axis.ticks = ggplot2::element_line(color = "white", linewidth = 0.2),
...
gline_theme <- function(base_size = 24, base_family = "") {
  ggplot2::theme_grey(base_size = base_size, base_family = base_family) + #%+replace%
    ggplot2::theme(
      ...
      panel.grid.major = ggplot2::element_line(color = "grey", linewidth = 0.1),
